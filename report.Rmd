...
ggplot(data = Cal[Sex == "Total"], mapping = aes(x = Year, y = Rate, colour = Disease)) + geom_line() + geom_point() + 
  ...
  labs(y = "Rate(%)", title = "Figure 1. Line plot of rate of each infectious disease from 2001 to 2020") 
```
...
ggplot() + 
  ...
  scale_x_continuous(breaks = seq(2001, 2020, 1)) +
  labs(y = "Rate(%)", title = "Figure 2. Boxplot of infection rate of Campylobacteriosis from 2001 to 2020") 
```
...
ggplot() + 
  ...
  labs(y = "Rate(%)", title = "Figure 3. Barplot of infection rate of Campylobacteriosis in different counties in 2019") 
```
...
leaflet(SB_2019) %>% 
  ...
  addControl("Figure 4. Leaflet map of the infection rate of Campylobacteriosis in 2019", position = "topright")
```
